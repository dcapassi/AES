...
// Multiplication matrix for decryption
const mul14 = 
[
...
function decryptCleanText(decryptedText){
  let array = matrixToArray(decryptedText.map(entry => entry.map( hex => hexToChar(hex))))
  ...
  function encryptedHex(cipherMatrix){
    let array = matrixToArray(cipherMatrix.map(entry => entry.map( hex => hex)))
    
    let encryptedTextArray = ""
    for (char of array){
      encryptedTextArray = encryptedTextArray + char
    }
  ...
  function encryptedTextArrayToMatrix(encryptedTextArray){
    let charArray = [];

    for (let i = 0, charsLength = encryptedTextArray.length; i < charsLength; i += 2) {
      charArray.push(encryptedTextArray.substring(i, i + 2));
    }
    return textMatrix = [
      [charArray[0],charArray[4],charArray[8],charArray[12]],
      [charArray[1],charArray[5],charArray[9],charArray[13]],
...
const inverseMixColumn = (state) => {
  ...
  //14 11 13 9
  tempIndex = getHexIndex(state[0][0])
  hex1 = mul14[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[1][0])
  hex2 = mul11[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[2][0])
  hex3 = mul13[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[3][0])
  hex4 = mul9[tempIndex[0]-1][tempIndex[1]-1]
  xor = stringToHex(hex1)^stringToHex(hex2)^stringToHex(hex3)^stringToHex(hex4)
  lineArray.push(intToHex(xor))
  ...
  //9 14 11 13
  tempIndex = getHexIndex(state[0][0])
  hex1 = mul9[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[1][0])
  hex2 = mul14[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[2][0])
  hex3 = mul11[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[3][0])
  hex4 = mul13[tempIndex[0]-1][tempIndex[1]-1]
  xor = stringToHex(hex1)^stringToHex(hex2)^stringToHex(hex3)^stringToHex(hex4)
  lineArray.push(intToHex(xor))
  ...
  //13 9 14 11
  tempIndex = getHexIndex(state[0][0])
  hex1 = mul13[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[1][0])
  hex2 = mul9[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[2][0])
  hex3 = mul14[tempIndex[0]-1][tempIndex[1]-1]
  tempIndex = getHexIndex(state[3][0])
  hex4 = mul11[tempIndex[0]-1][tempIndex[1]-1]
  xor = stringToHex(hex1)^stringToHex(hex2)^stringToHex(hex3)^stringToHex(hex4)
  lineArray.push(intToHex(xor))
  ...
  lineArray = []
    ...
    //11 13 9 14
    tempIndex = getHexIndex(state[0][0])
    hex1 = mul11[tempIndex[0]-1][tempIndex[1]-1]
    tempIndex = getHexIndex(state[1][0])
    hex2 = mul13[tempIndex[0]-1][tempIndex[1]-1]
    tempIndex = getHexIndex(state[2][0])
    hex3 = mul9[tempIndex[0]-1][tempIndex[1]-1]
    tempIndex = getHexIndex(state[3][0])
    hex4 = mul14[tempIndex[0]-1][tempIndex[1]-1]
    xor = stringToHex(hex1)^stringToHex(hex2)^stringToHex(hex3)^stringToHex(hex4)
    lineArray.push(intToHex(xor))
...
function subGenerationInverse(matrix) {
  let subMatrix = [];
  subMatrix = matrix.map(entry => entry.map(item => subByteInv(item)))
  return subMatrix;
}
...
function keyGeneration(key,round) {
  ...
  for (i = 0; i <= 3; i++){
    ...
    column.push(intToHex(xor))
  }

  keyMatrix.push(column)
  column = []
...
function shiftRowTransformationInv(matrix){
  return shiftMatrix = [
                       [matrix[0][0],matrix[0][1],matrix[0][2],matrix[0][3]],
...
function rotWord(vector) {
  ...
  function subByte(byte) {
    const rowColumnArray = getHexIndex(byte)
    const row = rowColumnArray[1]
    const Column = rowColumnArray[0]
    return sBox[Column][row]
    }

  function subByteInv(byte) {
      const rowColumnArray = getHexIndex(byte)
      const row = rowColumnArray[1]
      const Column = rowColumnArray[0]
      return inverseSBox[Column][row]
      }
...
let keyArray = generateKeyArray(key)
...
//ShiftRows
let shiftRowMatrix = shiftRowTransformation(subBytesMatrix)


//Mix Columns
let mixColumnsMatrix = mixColumns(shiftRowMatrix)


newStateMatrix = xorMatrix(mixColumnsMatrix,keyArray[i])
...
let cipherText = xorMatrix(shiftRowMatrix,keyArray[10])


return encryptedHex(cipherText)
...
function decryptAES128(encryptedText,keyPlainText) {
  ...
  let newStateMatrix = xorMatrix(cipherMatrix,keyArray[10])
  ...
  //SubBytes
  let subBytesMatrix = subGenerationInverse(shiftRowMatrix)
  
  
  newStateMatrix = xorMatrix(subBytesMatrix,keyArray[i])


  //Mix Columns
   newStateMatrix = inverseMixColumn(newStateMatrix)
  ...
  let decryptedText = xorMatrix(subBytesMatrix,keyArray[0])
  
  return decryptCleanText(decryptedText)
...
module.exports.encryptAES128 = encryptAES128;
module.exports.decryptAES128 = decryptAES128;
